  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "h" && (e.ctrlKey || e.metaKey)) {
        setShowBrowserList(!showBrowserList);
      }
    };
        ...
        {showBrowserList && (
          ...
          >
            {websitesList.map((site, i) => (
              <motion.div
                key={i}
                className="w-full h-[50vh] bg-[#1A1A1A]/90 backdrop-blur-md p-4 border border-neutral-200/10 shadow-md rounded-lg mx-auto absolute top-1/2 transform"
              ...
              >
                <div className='px-2 py-1.5 rounded-lg font-medium text-neutral-200/40 bg-neutral-800'>
                  <span role="img" aria-label={`search ${site.name} (read-only)`}>🔍</span>
